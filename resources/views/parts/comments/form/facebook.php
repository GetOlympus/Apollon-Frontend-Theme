...
return [
    ...
    'class_submit'  => !empty($data['opts']['class_submit']) ? $data['opts']['class_submit'] : 'uk-button uk-button-primary',
    'comment_field' => sprintf(
        ...
        // field
        '<textarea id="comment" name="comment" class="uk-textarea" aria-required="true" row="1"></textarea>'
    ),
    'fields'        => [
        'author' => sprintf(
            $data['_f']['field'],
            ...
            // field
            sprintf(
                '<input type="text" id="author" name="author" class="uk-input" value="%s" size="100" %s placeholder="%s"/>',
                // value
        ...
        'email' => sprintf(
            ...
            // field
            sprintf(
                '<input type="email" id="email" name="email" class="uk-input" value="%s" %s placeholder="%s"/>',
        ...
        'url' => !$data['website'] ? '' : sprintf(
            ...
            // field
            sprintf(
                '<input type="url" id="url" name="url" class="uk-input" value="%s" %s placeholder="%s"/>',
    ...
    'comment_notes_after'  => $data['htmltags'] ? sprintf($data['texts']['after'], ' <code>'.allowed_tags().'</code>') : '',
    'title_reply'          => '',
