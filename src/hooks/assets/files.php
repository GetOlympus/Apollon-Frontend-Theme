...
add_action('ol.apollon.assets_generate_cssfile', function () {
    ...
    // Create LESS file
    if (!is_file($procfile)) {
        do_action('ol.apollon.assets_generate_lessfile');
    }
...
add_action('ol.apollon.assets_generate_lessfile', function () {
    // Get base directory & the customier CSS
    $basedir  = apply_filters('ol.apollon.assets_base_dir', null);
    $cssfile  = $basedir.'main.css';
    $procfile = $basedir.'variables.less';
...
add_action('ol.apollon.assets_generate_targetfile', function ($filetype = 'less') {
    // Action
    $action = 'less' === $filetype ? 'less' : 'css';
    do_action('ol.apollon.assets_generate_'.$action.'file');
}, 10, 1);
